...
def get_n2c2_ids(data_path: str) -> list:
    ...
    ids = set(pd.read_csv(
        f"{data_path}/n2c2_ids/subject_ids.csv")['subject_id'].values)
    return list(ids)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input-csv', type=str,
                        required=True, help="Path to the notes.")
    parser.add_argument('--mimic-iii-ids', type=str,
                        required=True, help="Path to MIMIC-III notes to ignore")
    parser.add_argument('--mimic-iv-ids', type=str,
                        required=True, help="Path to MIMIC-IV notes to ignore")
    parser.add_argument('--output-csv', type=str, required=True,
                        help="Where to store the filtered notes.")
    ...
    ids_to_skip = list(df3.subject_id.values) + \
        list(df4.subject_id_mimic_iv.values)
